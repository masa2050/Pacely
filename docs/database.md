...
| next_menu       | jsonb                          | 次回練習メニュー(構造化データ。例: `{"menu_type": "ペース走", "distance_km": 10, "pace_sec_per_km": 330, "note": "..."}`。menu_typeは「ジョグ/ペース走/インターバル/ロング走/休養/ビルドアップ走」のいずれか、8-2②・docs/adr/021。ビルドアップ走のみ`segments`(区間ごとの`reps`/`distance_km`/`pace_sec_per_km`/`rest_sec`の配列)を持つ、docs/adr/022) |
