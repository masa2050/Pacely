...
## 2026-09-22 フェーズ6: 本番でGemini API(無料枠)の503・タイムアウトが発生

- **状況**: 本番で`GET /advices/latest`を実行すると、`503 UNAVAILABLE
  (This model is currently experiencing high demand)`や、しばらくすると
  `context deadline exceeded (Client.Timeout exceeded while awaiting headers)`
  が発生するようになった。ローカルの`go run`では同じAPIキー・同じモデルで成功した。
- **切り分け**: モデル名(`gemini-3.6-flash`)がGoogle側に実在するかを
  `GET https://generativelanguage.googleapis.com/v1beta/models?key=...`で確認し、
  存在することを確認(コード側の設定ミスではないと判断)。またRailwayの構造化ログで
  失敗リクエストの`latency`を確認したところ`30.935941634s`と、設定していた
  クライアントタイムアウト(30秒)ちょうどで打ち切られていた。即座に失敗せず
  タイムアウト上限まで律儀に待っていたことから、Railway側のネットワーク接続自体が
  詰まっているのではなく、Google側が応答を返せていないと判断した。
- **原因**: Gemini APIの無料枠は、ピーク時に他の無料枠利用者全員と処理能力を共有する
  仕様であり、有料ティアに比べて503・応答遅延が起きやすい(公開情報でも「無料枠は
  ピーク時に503が起きやすい」という記述が複数確認できた)。ADR013で「無料枠のまま
  本番運用する」と決めた際に想定していた「将来トラフィックが増え無料枠で不足する場合」
  に該当する状況だが、個人開発の検証程度のアクセス量でも発生し得ることが分かった。
- **対応**: 今回は有料ティアへの切り替え(クレジットカード登録が必要になる)は見送り、
  `GeminiClient`のHTTPクライアントタイムアウトを30秒→60秒に延長する暫定対応のみ行った
  (根本解決ではなく、頻発するようなら改めてADR013を再評価する前提)。
- **学び**: エラーメッセージだけで「原因不明のバグ」と判断せず、(1)設定値が実在するか
  外部APIに直接問い合わせて確認する、(2)ログのlatencyが「即失敗」か「上限まで待って失敗」かを
  見ることで、ネットワーク層の問題かアプリ層(呼び出し先サービス)の問題かを切り分けられる、
  という手順が有効だった。また、無料枠は「お金がかからない」だけでなく「品質・可用性が
  有料ティアより劣る」というトレードオフがあることを、ADRの記録を見返しながら再認識した。
